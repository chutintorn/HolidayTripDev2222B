...
export default function DepartReturnDateBox({
  ...
  // ✅ Mobile: one column (stack). Desktop: two columns
  const gridClass = "grid grid-cols-1 sm:grid-cols-2 gap-2";

  // ✅ Smaller rectangle look
  const boxClass = (active) =>
    "h-12 w-full rounded-md border border-slate-200 bg-white px-3 text-left shadow-sm hover:bg-slate-50 " +
    (active ? "ring-2 ring-sky-300" : "");

  const labelClass = "text-[12px] text-slate-500 font-medium";
  const valueClass = "text-[15px] text-slate-800 font-medium truncate";
        ...
        <div className={gridClass}>
          ...
          >
            ...
            <div className={valueClass}>
              {depText || (departLabel === "ไป" || departLabel === "วันไป" ? "เลือกวัน" : "Select")}
            </div>
          </button>
          ...
          >
            ...
            <div className={valueClass}>
              {retText || (returnLabel === "กลับ" || returnLabel === "วันกลับ" ? "เลือกวัน" : "Select")}
            </div>
          </button>
        </div>

        {/* Hidden RSuite toggle input (popup only) */}
        <div className="ibe-drbox-hidden-toggle" style={{ height: 0, overflow: "hidden" }}>
          <DateRangePicker
