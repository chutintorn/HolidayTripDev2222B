...
export default function MealPanel({ paxId, selectedOffers = [], rawDetail, t }) {
    ...
    return (
      <label className="flex items-start gap-3 rounded-lg border border-slate-200 bg-white p-2 hover:border-sky-300 cursor-pointer">
        ...
        <div className="min-w-0 flex-1">
          ...
          {!desc ? null : <div className="mt-0.5 text-[12px] text-slate-700 break-words">{desc}</div>}
        </div>
      </label>
    );
  }
  ...
  const emptyHint = (text) => (
    <div className="text-[12px] text-slate-500 bg-white border border-slate-200 rounded-lg p-3">{text}</div>
  );

  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
        {/* ✅ TOP ROW: Meal title + Depart/Return (replace the duplicate "Meal") */}
        <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-2 shrink-0">
              {legs.map((l) => (
                <button
                  ...
                  className={tabBtnClass(l.idx === legIndex)}
                >
                  {l.label}
                </button>
              ))}
        ...
        {/* ✅ FLIGHT/DATE pill row (like baggage) */}
        ...
        {/* ✅ CONFIRMED/SELECTING + Confirm/Release in SAME LINE */}
        <div className="mt-2 rounded-2xl border border-slate-200 bg-white px-3 py-2">
