...
function routeLabel(offer) {
  const o = String(offer?.origin || offer?.from || offer?.row?.origin || "").toUpperCase();
  const d = String(
    offer?.destination || offer?.to || offer?.row?.destination || ""
  ).toUpperCase();
  if (o && d) return `${o} → ${d}`;
  return "";
}
...
function getCurrency(offer, fallback = "THB") {
  return (
    ...
    offer?.priceCurrency ||
    offer?.row?.currency ||
    offer?.row?.currencyCode ||
    offer?.row?.priceCurrency ||
    fallback
  );
}
...
function pickPrice(offer) {
  ...
  const candidates = [
    ...
    // also try inside row (if passed)
    offer?.row?.fareAmountIncludingTax,
    offer?.row?.fareAmount,
    offer?.row?.totalFare,
    offer?.row?.totalPrice,
    offer?.row?.totalAmount,
  ];
...
// --- NEW: time getters (supports offer + offer.row) ---
function pickDepTime(offer) {
  return (
    offer?.departureTime ||
    offer?.departTime ||
    offer?.row?.departureTime ||
    offer?.row?.departTime ||
    ""
  );
}

function pickArrTime(offer) {
  return (
    offer?.arrivalTime ||
    offer?.arriveTime ||
    offer?.row?.arrivalTime ||
    offer?.row?.arriveTime ||
    ""
  );
}

function pickDuration(offer) {
  return offer?.duration || offer?.row?.duration || "";
}

function pickAircraft(offer) {
  return (
    offer?.aircraftDescription ||
    offer?.aircraft ||
    offer?.row?.aircraftDescription ||
    offer?.row?.aircraft ||
    ""
  );
}
 ...
 *
 * NOTE: Proceed/NEXT and Total are intentionally removed.
 */
export default function FlightSelectionSummary({
  ...
  // outbound = offers[0], inbound = offers[1]
  ...
  const OfferCard = ({ offer }) => {
    ...
    const jKey = offer?.journeyKey || offer?.row?.journeyKey;
    const flightNo =
      extractFlightNumberFromJourneyKey(jKey) ||
      offer?.flightNumber ||
      offer?.row?.flightNumber ||
      "";

    const iso =
      extractIsoFromJourneyKey(jKey) ||
      offer?.date ||
      offer?.row?.date ||
      offer?.row?.departureDate ||
      "";
    ...
    const depTime = pickDepTime(offer);
    const arrTime = pickArrTime(offer);
    const duration = pickDuration(offer);
    const aircraft = pickAircraft(offer);
    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex items-start justify-between gap-3">
          ...
          {/* Price + flight badge on the right */}
        ...
        {showDetails && (
          <div className="mt-3 rounded-lg border border-slate-200 bg-white p-2">
            ...
            <div className="mt-1 text-[12px] text-slate-700 break-words">
              ...
              {String(offer?.fareKey || offer?.row?.fareKey || "") || "-"}
            </div>
            <div className="mt-1 text-[12px] text-slate-700 break-words">
              <span className="font-semibold">{t.journeyKey}:</span>{" "}
              {String(offer?.journeyKey || offer?.row?.journeyKey || "") || "-"}
            </div>
          </div>
        )}
          ...
          blocks.map((b) => (
            <div key={b.key} className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {/* leg price in header line (small) */}
                {b.offer
                  ? (() => {
