...
function pickAirlinesFromRawDetail(rawDetail) {
  ...
  // ✅ 1) If rawDetail is already the "data" object (detail.data)
  // Example: rawDetail = { totalAmount, currency, airlines:[...] }
  const a0 = rawDetail?.airlines;
  if (Array.isArray(a0)) return a0;

  // ✅ 2) proxy mode shape: { data: { airlines: [...] } }
  const a1 = rawDetail?.data?.airlines;
  if (Array.isArray(a1)) return a1;

  // ✅ 3) nested proxy: { data: { data: { airlines: [...] } } }
  const a1b = rawDetail?.data?.data?.airlines;
  if (Array.isArray(a1b)) return a1b;

  // ✅ 4) full wrapper: { detail: { data: { airlines: [...] } } }
  const a2 = rawDetail?.detail?.data?.airlines;
  if (Array.isArray(a2)) return a2;

  // ✅ 5) extra nested: { detail: { data: { data: { airlines: [...] } } } }
  const a2b = rawDetail?.detail?.data?.data?.airlines;
  if (Array.isArray(a2b)) return a2b;

  // ✅ 6) sometimes: rawDetail is the full response you pasted:
  // { detail: { data: { airlines:[...] } } } already covered above,
  // but if someone passes rawDetail.detail directly:
  const a3 = rawDetail?.detail?.airlines;
  if (Array.isArray(a3)) return a3;

  // ✅ 7) sometimes: legs merged: { legs: [ { data:{airlines:[...]} }, ... ] }
...
export default function BaggagePanel({ paxId, selectedOffers = [], rawDetail, t = {} }) {
  ...
  const findServicesForLeg = (flightNumber) => {
    ...
    const bg = services.filter((s) =>
      /^BG\d{2}$/.test(String(s?.ssrCode || "").toUpperCase())
    );
    const sb = services.filter((s) =>
      /^SB\d{2}$/.test(String(s?.ssrCode || "").toUpperCase())
    );
    return { bg, sb };
  };

  if (!legs.length) {
    return <div className="text-sm text-slate-600">{t.noFlights ?? "No flight legs found."}</div>;
  }

  return (
    <div className="space-y-3">
      {legs.map((leg) => {
        // ✅ keep your working pattern (avoid big refactor now)
          ...
          <div key={leg.journeyKey} className="rounded-xl border border-slate-200 bg-white p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="font-extrabold text-slate-900">
                {title}{" "}
                <span className="text-slate-500 font-semibold">({leg.flightNumber || "—"})</span>
              </div>
            ...
            <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="font-bold text-slate-900 mb-2">{t.bagChecked ?? "Baggage (BGxx)"}</div>
                    ...
                    <label key={code} className="flex items-start gap-2 text-sm">
                      ...
                      <div className="min-w-0">
                        <div className="font-semibold text-slate-900">
                          {code} <span className="font-normal text-slate-600">{s?.description || ""}</span>
                        </div>
                        <div className="text-slate-600">{formatMoney(s?.amount, s?.currency || "THB")}</div>
                      </div>
                    </label>
                  );
                })}
            ...
            {/* SB group */}
            <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="font-bold text-slate-900 mb-2">{t.bagSpecial ?? "Special Bag (SBxx)"}</div>
