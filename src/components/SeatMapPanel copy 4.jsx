...
function extractSeatChargesForJourney(seatMapData, journeyKey, fareKey) {
  ...
  // ✅ fallback shapes (some proxies return directly)
  ...
  if (Array.isArray(seatMapData?.detail?.seatCharges)) return seatMapData.detail.seatCharges;
  ...
  const byFare =
    fareKey
      ? items.find((it) => it?.request?.fareKey === fareKey) ||
        items.find((it) => String(it?.request?.fareKey || "").includes(fareKey)) ||
        null
      : null;

  const byJourney =
    journeyKey
      ? items.find((it) => it?.request?.journeyKey === journeyKey) ||
        items.find((it) => String(it?.request?.journeyKey || "").includes(journeyKey)) ||
        null
      : null;

  const matched = byFare || byJourney || null;
  ...
  const seatCharges = pack?.seatCharges || pack?.seatCharge || pack?.seats || null;
