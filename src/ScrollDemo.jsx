...
// RightBox using useRef for auto-scroll
  ...
  const listItems = useMemo(
    () =>
      Array.from({ length: itemCount }).map((_, i) => (
        <li
          key={i}
          className="bg-purple-100 p-2 rounded border border-purple-300 text-purple-700"
        >
          🔵 Item {i + 1}
        </li>
      )),
...
// RightBox without useRef (manual scroll, with logging)
const RightBoxWithoutRef = ({ itemCount }) => {
  const boxRef = useRef(null);

  useEffect(() => {
    if (boxRef.current) {
      console.log('🔍 Manual Scroll info:', {
        scrollTop: boxRef.current.scrollTop,
        scrollHeight: boxRef.current.scrollHeight,
      });
    }
...
export default function ScrollDemo() {
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-100">
      ...
      <div className="text-center mb-4 space-x-2" aria-live="polite">
        <button
          onClick={() => setItemCount((prev) => Math.max(1, prev - 1))}
          className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-700 disabled:opacity-50"
      ...
      {/* FIX: sky-50 (Tailwind) instead of skyBlue-50 */}
      <div className="bg-sky-50 rounded-lg shadow border p-4 max-w-6xl mx-auto h-auto">
