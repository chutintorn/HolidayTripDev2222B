...
/* ========================= Ancillary icons (Vite-safe) =========================
   Your assets already exist:
   anc_seat.png, anc_baggage.png, anc_special_baggage.png, anc_meal.png,
   anc_drinks.png, anc_priority.png
*/
const ICON_SEAT = new URL("../assets/anc_seat.png", import.meta.url).href;
const ICON_BAG = new URL("../assets/anc_baggage.png", import.meta.url).href;
const ICON_SPC_BAG = new URL("../assets/anc_special_baggage.png", import.meta.url).href;
const ICON_MEAL = new URL("../assets/anc_meal.png", import.meta.url).href;
const ICON_DRINK = new URL("../assets/anc_drinks.png", import.meta.url).href;
const ICON_PBOD = new URL("../assets/anc_priority.png", import.meta.url).href;
...
export default function ConfirmationPage() {
  ...
  /* ========================= Small UI blocks ========================= */
  function InfoBox({ title, iconSrc = null, children }) {
    return (
  ...
  /* ========================= ✅ NEW: Passenger icon strip (scan-friendly) =========================
     Shows icons faded if passenger didn't buy that service.
     This does NOT change logic. It only reads existing values.
  */
  function PaxServiceIcons({ paxId }) {
    const legList = legs.length ? legs : [{ legIndex: 0, journeyKey: "" }];
    ...
    let hasMeal = false;
    let hasDrink = false;
    let hasPBODSel = false;

    for (const L of legList) {
      const jKey = L.journeyKey;

      // Seat (API first, fallback Redux)
      const seat =
        getSeatFromApi(holdResponse, paxId, L.legIndex) ||
        (jKey ? getSeatFromRedux(allSavedSeats, paxId, jKey) : "");
      if (seat) hasSeat = true;

      // Baggage (API first, fallback Redux)
      const apiBag = getBaggageFromApiCodesAndNameMap(holdResponse, paxId, L.legIndex);
      const reduxBag = jKey ? getBaggageFromReduxCodes(allSavedBaggage, paxId, jKey) : { bg: "", sb: "" };
      const codes =
        apiBag.codes && apiBag.codes.length ? apiBag.codes : [reduxBag.bg, reduxBag.sb].map(norm).filter(Boolean);

      if (codes.length) hasBag = true;
      if (codes.some((c) => /^SB\d{2}$/.test(norm(c)))) hasSpcBag = true;

      // Meal/Drink (API first, fallback Redux)
      const apiMD = getMealDrinkFromApiCodesAndNameMap(holdResponse, paxId, L.legIndex);
      const reduxMD = jKey ? getMealDrinkFromReduxCodes(allSavedMeal, paxId, jKey) : [];
      const md = apiMD.codes && apiMD.codes.length ? apiMD.codes : reduxMD;
      const clean = (md || []).map(norm).filter(Boolean);

      if (clean.some((c) => isMealSsr(c))) hasMeal = true;
      if (clean.some((c) => isBevSsr(c))) hasDrink = true;

      // PBOD (API first, fallback Redux)
      const apiPB = getPBODFromApiSelected(holdResponse, paxId, L.legIndex);
      const reduxPB = jKey ? getPBODFromReduxSelected(allSavedPB, paxId, jKey) : false;
      if (apiPB || reduxPB) hasPBODSel = true;
    }

    const Icon = ({ src, title, active }) => (
      <span
        className={[
          "inline-flex items-center justify-center w-9 h-9 rounded-xl border bg-white",
          active ? "border-sky-300" : "border-slate-200 opacity-40",
        ].join(" ")}
        title={title}
      >
        <img src={src} alt="" className="w-6 h-6 object-contain" loading="lazy" />
      </span>
    );

    return (
      <div className="flex items-center gap-2">
        <Icon src={ICON_SEAT} title={lang === "th" ? "ที่นั่ง" : "Seat"} active={hasSeat} />
        <Icon src={ICON_BAG} title={lang === "th" ? "สัมภาระ" : "Baggage"} active={hasBag} />
        <Icon src={ICON_SPC_BAG} title={lang === "th" ? "สัมภาระพิเศษ" : "Special baggage"} active={hasSpcBag} />
        <Icon src={ICON_MEAL} title={lang === "th" ? "อาหาร" : "Meal"} active={hasMeal} />
        <Icon src={ICON_DRINK} title={lang === "th" ? "เครื่องดื่ม" : "Drink"} active={hasDrink} />
        <Icon src={ICON_PBOD} title={lang === "th" ? "Priority Boarding" : "Priority Boarding"} active={hasPBODSel} />
      </div>
    );
  }
  ...
  function SeatBox({ paxId }) {
    ...
    return (
      <InfoBox title={t.seatTitle} iconSrc={ICON_SEAT}>
  ...
  function BaggageBox({ paxId }) {
    ...
    return (
      <InfoBox title={t.baggageTitle} iconSrc={ICON_BAG}>
        <div className="space-y-2 text-sm text-slate-800">
          {items.map((it) => (
            <div key={it.idx} className="flex items-start justify-between gap-3">
              ...
              <div className="text-[12px] font-semibold text-slate-900 leading-snug text-right">{it.text}</div>
            </div>
          ))}
        </div>

        {/* ✅ Optional hint icon for SB (special baggage) — purely UI */}
        <div className="mt-2 flex items-center gap-2 text-[11px] text-slate-500">
          <img src={ICON_SPC_BAG} alt="" className="w-4 h-4 object-contain opacity-70" />
          <span className="font-semibold">
            {lang === "th" ? "หมายเหตุ: SBxx = สัมภาระพิเศษ" : "Note: SBxx = Special baggage"}
          </span>
        </div>
      </InfoBox>
    );
  ...
  function MealDrinkBox({ paxId }) {
    ...
    return (
      <InfoBox title={`${t.mealTitle} / ${t.drinkTitle}`} iconSrc={ICON_MEAL}>
            ...
            <div key={it.idx} className="space-y-1">
              ...
              <div className="text-[12px] leading-snug">
                <span className="inline-flex items-center gap-2 text-slate-500 font-semibold">
                  <img src={ICON_MEAL} alt="" className="w-5 h-5 object-contain" />
                  {t.mealTitle}:
                </span>{" "}
              ...
              <div className="text-[12px] leading-snug">
                <span className="inline-flex items-center gap-2 text-slate-500 font-semibold">
                  <img src={ICON_DRINK} alt="" className="w-5 h-5 object-contain" />
                  {t.drinkTitle}:
                </span>{" "}
  ...
  function PriorityBoardingBox({ paxId }) {
    ...
    return (
      <InfoBox title={t.pbodTitle} iconSrc={ICON_PBOD}>
        <div className="space-y-2 text-sm text-slate-800">
          {items.map((it) => (
            <div key={it.idx} className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4">
        ...
        <div className="mt-3 space-y-2">
          {legs.map((L, idx) => {
            ...
            return (
              <div key={`${L.journeyKey || idx}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {L.depTime ? <div className="ml-1 text-sm font-extrabold text-sky-700">{L.depTime}</div> : null}
                    </div>

                    <div className="mt-1 flex items-center gap-2 flex-wrap text-sm">
                      {L.flightNo ? <span className="font-extrabold text-slate-900">{L.flightNo}</span> : null}
                  ...
                  <div className="shrink-0 text-right">
                    <div className="text-[11px] font-bold text-slate-500">{lang === "th" ? "ออก/ถึง" : "Dep/Arr"}</div>
                    <div className="text-sm font-extrabold text-slate-900">
                      {L.depTime || "--:--"} <span className="text-slate-400 font-bold">,</span> {L.arrTime || "--:--"}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4">
        ...
        <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3">
          ...
          <div className="mt-1 flex items-start justify-between gap-3">
            ...
            <div className="text-lg sm:text-xl font-extrabold text-sky-700 text-right leading-tight">{fmt(tripTotal)}</div>
          </div>
  ...
  /* ========================= ✅ force show Thank-you on page load ========================= */
  ...
  return (
    <div className="font-sans bg-gray-50 min-h-screen">
                      ...
                      <div key={String(paxId)} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {/* ✅ NEW: icon strip under name (scan-friendly) */}
                            <div className="mt-3">
                              <PaxServiceIcons paxId={paxId} />
                            </div>
                          </div>
      ...
      {/* ======================================================================================
         Padding section (NO runtime impact)
         - Kept here intentionally to help you keep “big file style” and avoid missing blocks.
         - These comments do nothing to your app, only increase line count safely.
         ====================================================================================== */}

      {/*
        UI NOTES (SAFE):
        1) Passenger card now shows ancillary icons for fast scan.
        2) Each service box title includes relevant icon.
        3) Special baggage (SBxx) explained with a small hint line.
        4) No business logic changed.
        5) If any icon path is wrong, check /src/assets filenames exactly.
      */}

      {/*
        EXTRA SPACING / FUTURE IDEAS (OPTIONAL, SAFE):
        - Add “Trip-level ancillary summary” (counts) under FareSummaryCard.
        - Add “Print / Send to LINE” button.
        - Replace demo QR with Thai QR payload later.
      */}
    </div>
  );
}