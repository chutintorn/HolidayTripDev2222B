...
export default function FareSidebar({
  ...
  /* ============================================================
     ...
     If selectedOffers empty, build 1-leg offer from state fareKey/journeyKey
     ============================================================ */
  const normalizedSelectedOffers = React.useMemo(() => {
    ...
    if (fk && jk) {
      return [{ fareKey: fk, journeyKey: jk, securityToken: token }];
    }
    return [];
  ...
  const addonsTotal = seatGrandTotal + baggageGrandTotal;
  const addonsCurrency = currency || seatCurrency || baggageCurrency || "THB";
    ...
    <aside className={"bg-white border border-slate-200 rounded-2xl p-4 h-fit " + (isLGUp ? "sticky top-20" : "")}>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-700">
                    <div>
                      <span className="font-semibold">fareKey:</span> <span className="break-all">{o.fareKey || "-"}</span>
                    </div>
                    <div className="mt-1">
                      <span className="font-semibold">journeyKey:</span> <span className="break-all">{o.journeyKey || "-"}</span>
                    </div>
                    {o.securityToken ? (
                      <div className="mt-1">
                        <span className="font-semibold">securityToken:</span> <span className="break-all">{o.securityToken}</span>
                      </div>
                    ) : null}
                  </div>
                </div>
      ...
      {fareSummary ? (
        <>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-[11px] font-extrabold text-slate-900">
                                {fmt(
                                  (Number(lg?.seat?.total || 0) || 0) + (Number(baggageByLeg?.[idx]?.baggage?.total || 0) || 0),
                                  addonsCurrency
                                )}
                              </div>
                            </div>
                                ...
                                {seatsPerPaxForJourney(allSavedSeats, lg.journeyKey).length === 0 ? (
                                  ...
                                  seatsPerPaxForJourney(allSavedSeats, lg.journeyKey).map((x) => (
                                    ...
                                    >
                                      ...
                                      <div className="font-extrabold text-slate-900">{fmt(x.total, x.currency || seatCurrency)}</div>
                                    </div>
                                  ))
                                )}
                              </div>
          ...
          {/* ✅ Continue: payload patch (ONE-WAY SAFE + ONLY BG/SB + Seat) */}
          <button
                  ...
                  passengerInfos: (payload.passengerInfos || []).map((paxInfo, paxIdx) => {
                    ...
                    const newFlightFareKey = (paxInfo.flightFareKey || []).map((ffk) => {
                      ...
                      return {
                        ...
                        // ✅ Put ONLY what user selected on web page (BG/SB)
