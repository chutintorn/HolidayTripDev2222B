...
/* ========================= Helpers: PBOD SSR only ========================= */
function isPBODSsrCode(code) {
  const s = String(code || "").trim().toUpperCase();
  return s === "PBOD";
}

/**
 * priority boarding expected:
 *  root[paxId][journeyKey] = { pbod: { ssrCode:"PBOD", amount, currency, vat, description, flightNumber } } OR null
 */
function buildPBODExtraServices(pbLeg, journeyKey) {
  if (!pbLeg) return [];
  const out = [];

  const pbod = pbLeg?.pbod;
  const ssr = String(pbod?.ssrCode || "").trim().toUpperCase();
  if (!isPBODSsrCode(ssr)) return out;

  const flightNumber =
    String(pbod?.flightNumber || "").trim().toUpperCase() || extractFlightNumberFromJourneyKey(journeyKey);
  if (!flightNumber) return out;

  out.push({ flightNumber, ssrCode: "PBOD" });
  return out;
}
...
/* ========================= Helpers: PBOD totals (effective) ========================= */
function pbodTotalForJourney(rootPB, journeyKey) {
  const jKey = String(journeyKey || "");
  let amount = 0;
  let vat = 0;
  let currency = "THB";
  ...
  for (const paxId of Object.keys(root)) {
    const leg = root?.[paxId]?.[jKey];
    const pbod = leg?.pbod;
    if (!pbod?.ssrCode) continue;
    if (!isPBODSsrCode(pbod?.ssrCode)) continue;

    count += 1;
    amount += Number(pbod?.amount || 0) || 0;
    vat += Number(pbod?.vat || 0) || 0;
    currency = pbod?.currency || currency;
  }

  return { amount, vat, total: amount + vat, currency, count };
}

function pbodPerPaxForJourney(rootPB, journeyKey) {
  const jKey = String(journeyKey || "");
  const root = rootPB || {};
  const out = [];
  ...
  for (const paxId of Object.keys(root)) {
    ...
    const currency = pbod?.currency || "THB";
    const amt = Number(pbod?.amount || 0) || 0;
    const vat = Number(pbod?.vat || 0) || 0;
      ...
      items: [
        {
          ...
          amount: amt + vat,
          currency,
          name: pbod?.description || "PRIORITY BOARDING",
        },
      ],
      total: amt + vat,
      currency,
...
function buildEffectivePBRoot(savedRoot, draftRoot) {
  const saved = savedRoot || {};
  const draft = draftRoot || {};
  const paxIds = new Set([...Object.keys(saved), ...Object.keys(draft)]);
...
export default function FareSidebar({
  ...
  // ✅ Safe read PBOD saved + draft (no selector import)
  ...
  const allDraftPB = useSelector((s) => {
    return (
      s?.priorityBoardingSelection?.draft ||
      s?.priorityBoardingSelectionSlice?.draft ||
      s?.priorityBoarding?.draft ||
      s?.pbodSelection?.draft ||
      {}
    );
  });
  ...
  // ✅ effective PBOD (draft preferred if exists)
  const effectivePB = React.useMemo(() => {
    return buildEffectivePBRoot(allSavedPB, allDraftPB);
  }, [allSavedPB, allDraftPB]);
  ...
  // ✅ PBOD totals (use effective PBOD)
  const pbByLeg = React.useMemo(() => {
    const out = [];
    for (const lg of legs) {
      const jKey = lg?.journeyKey || "";
      const pb = jKey
        ? pbodTotalForJourney(effectivePB, jKey)
        : { amount: 0, vat: 0, total: 0, currency: currency || "THB", count: 0 };
      out.push({ ...lg, pb });
    }
    return out;
  }, [legs, effectivePB, currency]);

  const pbGrandTotal = React.useMemo(() => {
    return pbByLeg.reduce((sum, x) => sum + (Number(x?.pb?.total || 0) || 0), 0);
  }, [pbByLeg]);

  const pbCurrency = React.useMemo(() => {
    const c = pbByLeg?.[0]?.pb?.currency;
    return c || currency || "THB";
  }, [pbByLeg, currency]);

  // ✅ addons total now include PBOD
  const addonsTotal = seatGrandTotal + baggageGrandTotal + mealGrandTotal + pbGrandTotal;
  const addonsCurrency = currency || seatCurrency || baggageCurrency || mealCurrency || pbCurrency || "THB";
  ...
  const L = {
    ...
    meal: t?.meal ?? "Meal",
    pbod: t?.priorityBoard ?? t?.pbod ?? "Priority Boarding",
    pbodPerPassenger: t?.pbodPerPassenger ?? "Priority boarding per passenger",
    ...
    <aside className={"bg-white border border-slate-200 rounded-2xl p-4 h-fit " + (isLGUp ? "sticky top-20" : "")}>
      ...
      {fareSummary ? (
        <>
            ...
            {!showDetails ? null : (
              <div className="px-3 pb-3 space-y-3">
                <div className="grid grid-cols-[1fr_auto] gap-y-2 text-sm">
                  ...
                  <div className="text-slate-600">{L.pbod}</div>
                  <div className="font-semibold">{fmt(pbGrandTotal, pbCurrency)}</div>
                    ...
                    <div className="space-y-2">
                      {seatByLeg.map((lg, idx) => {
                        ...
                        const legTotal =
                          ...
                          (Number(pbByLeg?.[idx]?.pb?.total || 0) || 0);
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-[11px] font-extrabold text-slate-900">{fmt(legTotal, addonsCurrency)}</div>
                            </div>
                            ...
                            <details className="mt-2">
                              ...
                              <div className="mt-2 space-y-2">
                                {pbodPerPaxForJourney(effectivePB, lg.journeyKey).length === 0 ? (
                                  <div className="text-[11px] text-slate-500">{L.none}</div>
                                ) : (
                                  pbodPerPaxForJourney(effectivePB, lg.journeyKey).map((p) => (
                                    <div
                                      key={`${lg.journeyKey}-pb-${p.paxId}`}
                                      className="rounded-lg border border-slate-200 bg-white p-2"
                                    >
                                      <div className="flex items-center justify-between text-[11px]">
                                        <div className="text-slate-700 font-extrabold">Pax {p.paxId}</div>
                                      ...
                                      <div className="mt-1 space-y-1">
                                        {p.items.map((it, idx2) => (
                                          <div
                                            key={`${lg.journeyKey}-pb-${p.paxId}-${it.ssrCode}-${idx2}`}
                                            className="flex items-center justify-between text-[11px]"
                                          >
                                            <div className="text-slate-700">
                                              <span className="font-extrabold">{it.ssrCode}</span>
          ...
          {/* ✅ Continue: payload patch (ONE-WAY SAFE + ONLY BG/SB + Seat + MH/MS/BEV + PBOD) */}
          <button
              ...
              try {
                ...
                const patched = {
                  ...
                  passengerInfos: (payload.passengerInfos || []).map((paxInfo, paxIdx) => {
                    ...
                    // ✅ use effective PBOD (draft preferred if exists)
                    const effPBByJourney = paxId ? effectivePB?.[String(paxId)] : null;
                    ...
                    const newFlightFareKey = (paxInfo.flightFareKey || []).map((ffk) => {
                      ...
                      // PBOD from effective store (ONLY PBOD)
                      const pbLeg = effPBByJourney?.[String(jKey)] || null;
                      const pbExtras = buildPBODExtraServices(pbLeg, jKey);

                      const extraService = dedupeExtraServices([
                        ...(bagExtras || []),
                        ...(mealExtras || []),
                        ...(pbExtras || []),
                      ]);
                      ...
                      return {
                        ...
                        // ✅ Put ONLY what user selected on web page (BG/SB/MH/MS/BEV/PBOD)
                        extraService,
